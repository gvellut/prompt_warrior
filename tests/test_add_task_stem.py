...
class AddTaskStemTests(unittest.TestCase):
    def test_top_level_uses_prefix_after_last_instead_of_filling_gap(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            stem = choose_task_stem(
                _make_app_ctx(Path(temp_dir)),
                document=document,
                parent_task_index=None,
                label_component="new_task",
            )

        self.assertEqual(stem, "W_new_task")

    def test_child_uses_prefix_after_last_in_custom_global_order(self) -> None:
        document = _parse_document(
            [
                "- [Parent](P_parent.md)\n",
        ...
        self.assertEqual(stem, "P.Ba_new_child")
