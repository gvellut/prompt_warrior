...
from ..cli_params import argument_task_reference, pass_app_context
from ..constants import PWAR_KEEP_CHILDREN
from ..core import (
...
from ..models import AppContext, BulletType
...
def register(cli: click.Group) -> None:
    @cli.command(
        ...
        help=(
            "Delete a task by reference and optionally "
            "keep/promote its direct children."
        ),
    )
    ...
    @click.option(
        ...
        help=(
            "Delete only the target task and "
            "promote direct children to the target level."
        ),
    )
    @pass_app_context
    def delete(app_ctx: AppContext, task_ref: str, keep_children: bool) -> None:
        ...
        if keep_children:
            ...
            remove_task_files(
                app_ctx, [document.tasks[index] for index in delete_indices]
            )
