...
from ..models import ACTIONABLE_BULLETS, AppContext, BulletType
...
def register(cli: click.Group) -> None:
    @cli.command(
        ...
        help=(
            "Activate the next task and copy its markdown file content "
            "to the clipboard."
        ),
    )
    @pass_app_context
    def next_task(app_ctx: AppContext) -> None:
        ...
        if next_candidate is None:
            ...
            raise PromptWarriorError(
                "No remaining '-' or '?' top-level task to activate."
            )
